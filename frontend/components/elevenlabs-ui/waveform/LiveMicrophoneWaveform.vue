...
watch(
  () => props.dragOffset,
  (val) => {
    if (val !== undefined) currentDragOffset.value = val;
  }
);
...
function playScrubSound(position: number, direction: number) {
  if (!props.enableAudioPlayback || !audioBuffer.value || !audioContextRef.value) return;
...
function handleMouseDown(e: MouseEvent) {
  if (props.active || history.value.length === 0) return;
...
function handleMouseMove(e: MouseEvent) {
  ...
  const now = Date.now();
  if (props.enableAudioPlayback && audioBuffer.value && now - lastScrubTime > 50) {
    lastScrubTime = now;
...
function handleMouseUp() {
  if (isDragging.value) {
    ...
    if (props.enableAudioPlayback && audioBuffer.value) {
      ...
      const rightmostBarIndex = Math.max(0, Math.min(maxBars - 1, maxBars - 1 - offsetBars));
      const audioPosition = (rightmostBarIndex / maxBars) * audioBuffer.value.duration;
      playFromPosition(Math.max(0, Math.min(audioBuffer.value.duration - 0.1, audioPosition)));
    }
