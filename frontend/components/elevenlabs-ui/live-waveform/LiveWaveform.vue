...
onMounted(() => {
  ...
  onUnmounted(() => {
    ...
    if (audioState.processingAnimationId) cancelAnimationFrame(audioState.processingAnimationId);
  });
});

// Processing Animation Logic
watch(
  () => [props.processing, props.active, props.mode],
  () => {
    ...
    if (props.processing && !props.active) {
        ...
        if (props.mode === 'static') {
          ...
          for (let i = 0; i < barCount; i++) {
            ...
            if (audioState.lastActiveData.length > 0 && audioState.transitionProgress < 1) {
              ...
              finalValue =
                lastValue * (1 - audioState.transitionProgress) +
                processingValue * audioState.transitionProgress;
            }
      ...
      // Fade out logic
      const hasData =
        props.mode === 'static' ? audioState.staticBars.length > 0 : audioState.history.length > 0;
...
onMounted(() => {
  ...
  const animate = (currentTime: number) => {
    ...
    if (audioState.needsRedraw || props.active) {
      ...
      for (
        let i = 0;
        i < barCount && i < (props.mode === 'static' ? dataToRender.length : Infinity);
        i++
      ) {
        let value = 0.1;
        let x = 0;
        ...
        if (props.barRadius > 0) {
          ...
          if ('roundRect' in ctx) {
            ...
            ).roundRect(x, y, props.barWidth, barHeight, props.barRadius);
          } else {
